...
make_A_marine_spring_chinook <- function(
    SAR,                   # total smolt-to-adult return from EDT (DI)
    prespawn_surv,          # pre-spawn survival p (DI)
    ocean_surv = c(0.6, 0.7, 0.8, 0.9, 0.9),  # Table 2 Spring Chinook
    b = c(b2 = 0.005, b3 = 0.097, b4 = 0.6, b5 = 0.8) # Table 2 Spring Chinook
) {
...
#===== base sim function ===================
simulate_spring_chinook <- function(
    nyears,
  ...
  A <- make_A_marine_spring_chinook(
    SAR = SAR,
...
# loop sim function
simulate_spring_chinook_tv_const_ocean <- function(
    nyears,
    init = c(Spawners=500, Smolts=0, O1=0, O2=0, O3=0, O4=0, O5=0),
    ...
    b = c(b2 = 0.005, b3 = 0.097, b4 = 0.6, b5 = 0.8) # constant
) {
  ...
  for (t in 1:nyears) {
    ...
    A_t <- make_A_marine_spring_chinook(
      SAR = SAR_t,
...
dat <- read.csv("C:/Users/Blake.Hamilton/OneDrive - Kleinschmidt Associates/Projects/Chehalis/Pop ass model/SEPA LCM/Chinook/data/Master SEPA EDT Outputs.csv",
                check.names = FALSE)
#we only care about above crim and rbf to crim spring chinook and near for this
  ...
  for(sc in scenarios){
    ...
    for(j in 1:500)
    {
      ...
      params <- data.frame(Year = 1:nyears, 
                           ...
                           b2=0.005, b3=0.097, b4=0.6, b5=0.8)
      ...
      sim <- simulate_spring_chinook_tv_const_ocean(
        nyears = nyears,
      ...
      if(j == 1){plot(sim$Year, sim$Spawners, type="l", xlab="Year", ylab="Spawners", ylim = c(0,1000))}else{
